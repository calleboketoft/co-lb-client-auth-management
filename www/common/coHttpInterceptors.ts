  ...
  function httpReqResHandler ($q, $window, $injector) {
    return {
      request (config) {
        ...
        // TODO make this global config more maintainable
        var urlStart = config.url.slice(0, 4);
        if ($window.appConfigParams.apiUrl && (urlStart === 'api/' || urlStart === '/api')) {
            config.url = $window.appConfigParams.apiUrl + config.url;
        }
